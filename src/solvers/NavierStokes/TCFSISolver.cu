...
void TCFSISolver<memoryType>::stepTime()
{
	...
	NavierStokesSolver<memoryType>::qk = NavierStokesSolver<memoryType>::q; 
	...
	NSWithBody<memoryType>::B.uBk = NSWithBody<memoryType>::B.uB; //guess uB at n+1
	//NSWithBody<memoryType>::B.vBk = NSWithBody<memoryType>::B.vB; //guess vB at n+1
	NSWithBody<memoryType>::B.xk = NSWithBody<memoryType>::B.x;
	//NSWithBody<memoryType>::B.yk = NSWithBody<memoryType>::B.y;
	//guess that the body will keep moving the same way it was in the previous time step
	//might need to impose a check to make sure things don't run away from thier positions.
	...
	else
	{
		do
		{
			...
			//std::cout<<k<<"\t"<<std::cout<<NSWithBody<memoryType>::B.uBkp1[0]<<NSWithBody<memoryType>::B.forceX[0]<<std::endl;
