...
/*__global__
void force_velocity_x(double *force_dudx, double *uB, double *image_point_u,
						double *body_intercept_x, double *body_intercept_y, double *image_point_x, double *image_point_y,
						double *bx, double *by, double *xu, double *yu, int *hybridTagsUV,
						int *i_start, int *j_start, int width, int height, int totalPoints, int nx, int ny, double midX, double midY)
{
	//initialise
	int idx = threadIdx.x + blockDim.x * blockIdx.x;
	...
	for (int i = I0; i<=If; i++)
	{
		for(int j=J0;j<=Jf;j++)
		{
			...
			{
				theta = asin((body_intercept_y[iu]-midY)/sqrt(pow(body_intercept_x[iu]-midX,2)+pow(body_intercept_y[iu]-midY,2)));
				if (body_intercept_x[iu]<midX)
				{
					theta = M_PI-theta;
				}
				if (thetaNode > M_PI*5/4 || thetaNode < -M_PI/4)
	...
	//interp for node
	double	l1 = sqrt(pow(body_intercept_x[minID]-image_point_x[minID],2) + pow(body_intercept_y[minID]-image_point_y[minID],2)),
			l2 = sqrt(pow(body_intercept_x[maxID]-image_point_x[maxID],2) + pow(body_intercept_y[maxID]-image_point_y[maxID],2)),
			du1 = (image_point_u[minID] - uB[0])/l1,
			du2 = (image_point_u[maxID] - uB[0])/l2;
	force_dudx[idx] = du1 + (du2 - du1) * (thetaNode-min) / (max-min);
}*/
__global__
