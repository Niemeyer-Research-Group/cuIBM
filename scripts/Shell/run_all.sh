...
do
	...
	$CUIBM_DIR/scripts/python/plotVelocity.py -folder $CUIBM_DIR/validation/cavity/Re$var2 >/dev/null
done
...
do
	echo "Plotting lid driven cavity flow for Reynolds number "$var4
	$CUIBM_DIR/scripts/python/plotVelocity.py -folder $CUIBM_DIR/validation/cavity/Re$var4 >/dev/null
done
