...
clear
close all
M = dlmread('/scratch/src/cuIBM/validation/luo/test/interp_testY.csv','\t',1,0); %start on second row to avoid headers
...
for i =1:length(M)
    ...
    %plot3([X(1) X(7)], [Y(1) Y(7)], [Z(1),ip_u],'-.') %line between gn and ml ip
end
    
axis square
legend('Ghost Node','Boundary Intercept','cpp ip','ml ip','Interpolation corner')
